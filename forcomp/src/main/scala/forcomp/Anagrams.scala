...
object Anagrams {
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = 
    loadDictionary.groupBy((elem: Word) => wordOccurrences(elem)) withDefaultValue List()
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def occurrAnagrams(occurrences: Occurrences): List[Sentence] = {
	    if (occurrences == Nil) List(List())
	    else { 
	      (for {
	        comb <- combinations(occurrences)
	        words <- dictionaryByOccurrences(comb)
	        rest <- occurrAnagrams(subtract(occurrences, comb))
	        if (words != List())
	      } yield words :: rest).toList
	    }
    }
    
    if (sentence == Nil) List(List())
    else { 
      occurrAnagrams (sentenceOccurrences(sentence))
  	}
  }
